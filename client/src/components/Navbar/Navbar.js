     ...
     <div className={!getLocation() ?`font-Inter font-semibold w-full flex h-fit py-[10px]  lg:py-5 px-1 lg:pr-8 lg:pl-8 bg-navColor justify-between text-white items-center` : 'hidden'}>
        ...
        <div className='w-full flex flex-col gap-3 lg:hidden'>
            <div className='flex items-center justify-around'>
                ...
                className='w-[10%] relative'>
                    ...
                    {
                (cartList.length === 0) ?''
                :<div className='absolute -top-[6px] right-[6px] z-40 bg-red-500 w-4 h-4 rounded-full flex justify-center items-center text-xs '>{cartProductCounter}</div>
            }
                </div>
                {
        ...
        className={open ?'lg:hidden fixed h-full top-0 left-0 right-0 bottom-0 ease-in-out duration-300 bg-[rgba(0,0,0,0.7)] z-40':'lg:hidden'}>
            ...
            className={open ?' shadow-xl w-[65%] h-full z-40 bg-white text-gray-600 items-center fixed top-0 left-0 ease-in-out duration-300 shadow-black': 'fixed top-0 left-[-100%] '}>
                ...
                <li
                onClick={(e)=>handleNavigation('/order-page',e)} 
                ...
                <li
                onClick={(e)=>handleNavigation("/user-cart",e)} 
                className='py-[10px] pl-4  flex gap-2 items-center'>
                    <div><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#808080" className="w-5 h-5">
